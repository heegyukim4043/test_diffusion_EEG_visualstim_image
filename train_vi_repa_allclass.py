"""
train_vi_repa_allclass.py
─────────────────────────────────────────────────────────────────────────────
preproc_data_vi 데이터셋 / 9-class / REPA 손실 포함 EEG→Image 확산 모델 학습
★ subject-wise 모드: 각 피험자마다 독립적인 모델을 학습·저장한다.

▸ 모델   : model_128_eegonly_transformer_repa.EEGDiffusionModel128
▸ 손실   : ε-pred MSE + 재구성(L1) + SSIM + REPA(ResNet18 feature alignment)
▸ 데이터 : preproc_data_vi/subj_XX.mat  (X: ch×T×trial, y: trial)
▸ 이미지 : preproc_data_vi/images/01.png ~ 09.png
▸ 학습   : 피험자별 독립 모델 (subject-wise)
▸ 추론   : sample_vi_repa_allclass.py 사용

사용 예시
─────────
# 단일 피험자 (subj 03)
python train_vi_repa_allclass.py --subject_ids 3

# 범위 지정 (1~10번 피험자를 각각 독립 모델로 학습 → 10개 모델)
python train_vi_repa_allclass.py --subject_ids 1-10

# 개별 지정 (4개 독립 모델)
python train_vi_repa_allclass.py --subject_ids 1,3,5,7

# 혼합 (1~5번 + 8번 + 10번 → 7개 독립 모델)
python train_vi_repa_allclass.py --subject_ids 1-5,8,10

# 전체 피험자 (각각 독립 모델)
python train_vi_repa_allclass.py --subject_ids all

# 주요 하이퍼파라미터 조정
python train_vi_repa_allclass.py --subject_ids 1-10 \\
    --epochs 100 --batch_size 32 --lr 1e-4 \\
    --lambda_percept 0.05 --lambda_rec 0.02 --lambda_ssim 0.05 \\
    --num_timesteps 200 --base_channels 64 --ch_mult 1,2,4,4
"""
...
def parse_subject_ids(s: str, all_mat: list) -> list:
    ...
    if s == "all":
        ids = []
        for p in all_mat:
            base = os.path.basename(p)           # subj_01.mat
...
class EEGImageDataset9Class(Dataset):
    """
    EEG→Image 데이터셋 (preproc_data_vi 포맷).
    ...
    def __getitem__(self, idx):
        ...
        label_0based = label_1based - self.cls_min  # 그룹 내 0-based
...
def train_one_subject_group(
    ...
    group_id : 1 → cls 1-3 | 2 → cls 4-6 | 3 → cls 7-9
               0 → 전체 9-class 모드 (group_ids="0" 또는 미지정)
    """
    ...
    save_dir = os.path.join(
        args.ckpt_root,
        f"{timestamp}_subj{subj_str}_{group_tag}_{args.img_size}"
    )
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"Subj {subj_str} | {group_tag} REPA Val Loss")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="preproc_data_vi REPA EEG→Image 학습 (9-class, 128×128)"
    )

    # 데이터
    parser.add_argument("--data_root", type=str, default="./preproc_data_vi",
                        help="preproc_data_vi 폴더 경로")
    ...
    parser.add_argument("--lambda_percept", type=float, default=0.05,
                        help="REPA feature-alignment 손실 가중치")
    ...
    for (sid, gid), d in results.items():
        ...
        print(f"  subj {sid:02d} [{gtag}] → {status}")
