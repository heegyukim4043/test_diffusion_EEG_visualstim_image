...
def get_group_all_indices(mat_path: str, cls_low: int, cls_high: int):
    mat = loadmat(mat_path)
    y = mat["y"].squeeze().astype(np.int64)
    mask_group = (y >= cls_low) & (y <= cls_high)
    return np.where(mask_group)[0]
...
def main(args):
    ...
    set_seed(args.seed)
    if args.all_trials:
        test_idx = get_group_all_indices(mat_path, cls_low, cls_high)
    else:
    ...
    with torch.no_grad():
        ...
        if args.no_gt:
            print(f"Saved {global_sample_idx} images (GEN only).")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_root", type=str,
                        default="./checkpoints_subj128_group_eegonly_tf_repa")
    parser.add_argument("--ckpt_dir", type=str, default=None)

    parser.add_argument("--samples_root", type=str,
                        default="./samples_subj128_group_eegonly_tf_repa")
    ...
    parser.add_argument("--all_trials", action="store_true", default=False,
                        help="generate for all trials in the group (ignore split)")
    parser.add_argument("--no_gt", action="store_true", default=False,
                        help="skip loading/saving GT images (GEN only)")
