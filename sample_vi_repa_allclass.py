"""
sample_vi_repa_allclass.py
─────────────────────────────────────────────────────────────────────────────
train_vi_repa_allclass.py 로 학습한 모델에서 이미지를 생성한다.
...
─────────
# 단일 피험자
python sample_vi_repa_allclass.py --subject_ids 1

# 범위 지정 (1~10번 피험자)
python sample_vi_repa_allclass.py --subject_ids 1-10

# 개별 지정
python sample_vi_repa_allclass.py --subject_ids 1,3,5,7

# 혼합 (1~5번 + 8번 + 10번)
python sample_vi_repa_allclass.py --subject_ids 1-5,8,10

# 전체 피험자
python sample_vi_repa_allclass.py --subject_ids all

# 특정 체크포인트 직접 지정 + DDIM
python sample_vi_repa_allclass.py \\
    --subject_ids 1-10 \\
    --ckpt_path ./checkpoints_vi_repa_9cls/<dir>/best.pt \\
    --use_ddim --sample_steps 50 --guidance_scale 3.0

# 학습 분할의 test set 만 생성
python sample_vi_repa_allclass.py --subject_ids 1-10 --split test

# 피험자 전체 trial 생성 (GT 저장 없이)
python sample_vi_repa_allclass.py --subject_ids all --split all --no_gt
"""
...
def find_best_ckpt(ckpt_root: str, subj_tag: str, group_tag: str = "") -> str | None:
    ...
    pattern = f"*subj{subj_tag}*{group_tag}*" if group_tag else f"*subj{subj_tag}*"
    cands = sorted(glob.glob(os.path.join(ckpt_root, pattern)))
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset (추론 전용 — GT 이미지 선택적 로드)
# ─────────────────────────────────────────────────────────────────────────────

class EEGInferenceDataset(Dataset):
    """
    mat 파일의 특정 trial 집합에 대한 EEG + (선택적) GT 이미지 로드.
    """
    ...
    def __init__(
        ...
        self.eeg    = torch.from_numpy(X.astype(np.float32)).permute(2, 0, 1)  # (trial,ch,T)
    ...
    def __getitem__(self, idx):
        ...
        eeg          = self.eeg[t_idx]               # (ch, T)
        label_1based = int(self.labels[t_idx])        # 1~9
        label_0based = label_1based - 1               # 0~8
...
def get_split_indices(mat_path: str, split: str, seed: int = 42, class_filter: list = None):
    ...
    for cls in classes:
        ...
        if len(cls_trials) == 0:
            continue
        rng = np.random.RandomState(seed + cls)   # s_idx=0 (단일 피험자)
...
def sample(args, subject_id: int, group_id: int = 0):
    ...
    if ckpt_path is None:
        ...
        if ckpt_path is None:
            raise FileNotFoundError(
                f"체크포인트를 찾을 수 없습니다. "
                f"--ckpt_path 로 직접 지정하거나 --ckpt_root 를 확인하세요.\n"
                f"  탐색: *subj{subj_str}*{group_tag}*"
            )
    ...
    print(f"[INFO] split={args.split}, trials={len(indices)}, group={group_tag or '9cls'}")

    ds = EEGInferenceDataset(
        mat_path, img_root, indices,
        img_size=img_size, load_gt=(not args.no_gt)
    )
    loader = DataLoader(
    ...
    gtag_str  = group_tag if group_tag else "9cls"
    out_dir   = os.path.join(args.samples_root, ckpt_name, f"subj{subj_str}_{gtag_str}_{args.split}")
    ...
    with torch.no_grad():
        for eeg, gt_img, label_0, label_1, trial_idx in loader:
            ...
            # 그룹 내 0-based 라벨: label_1(1~9) - cfg_cls_min
            ...
            for i in range(eeg.size(0)):
                ...
                gen_name = f"subj{subj_str}_trial{tidx:04d}_cls{lbl:02d}_{mode}_GEN.png"
                to_pil(gen_denorm[i]).save(os.path.join(out_dir, gen_name))

                if not args.no_gt:
                    gt_name = f"subj{subj_str}_trial{tidx:04d}_cls{lbl:02d}_GT.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="preproc_data_vi REPA EEG→Image 생성 (9-class)"
    )

    # 데이터
    parser.add_argument("--data_root",    type=str, default="./preproc_data_vi")
    parser.add_argument("--subject_ids",  type=str, required=True,
                        help="예: '1' / '1-10' / '1,3,5' / '1-5,8,10' / 'all'")
    parser.add_argument("--split",        type=str, default="test",
                        choices=["train", "val", "test", "all"],
                        help="trial 분할 선택 (학습 때와 동일한 시드 사용)")
    parser.add_argument("--img_size",    type=int, default=128)

    # 체크포인트
    parser.add_argument("--ckpt_root",  type=str, default="./checkpoints_vi_repa_9cls",
                        help="자동 탐색할 루트 폴더")
    parser.add_argument("--ckpt_path",  type=str, default=None,
                        help="체크포인트 파일을 직접 지정 (우선 적용)")
    ...
    parser.add_argument("--use_ddim",       action="store_true", default=False,
                        help="DDIM 샘플링 사용 (기본: DDPM)")
    parser.add_argument("--sample_steps",   type=int, default=200,
                        help="DDPM 전체 T 또는 DDIM 스텝 수")
    parser.add_argument("--guidance_scale", type=float, default=2.5)
    parser.add_argument("--ddim_eta",       type=float, default=0.0,
                        help="DDIM η (0=결정론적, 1=DDPM과 동일)")

    # 출력
    parser.add_argument("--samples_root", type=str, default="./samples_vi_repa_9cls")
    ...
    parser.add_argument(
        ...
        help=(
            "생성할 클래스 그룹. 예: '1' / '1,2,3' / 'all' / '0'(전체 9-class)\n"
    ...
    for sid in subject_list:
        ...
        for gid in group_list:
            ...
            print(f"\n{'='*60}")
            print(f"  Subject {sid:02d}  [{gtag}]")
            print(f"{'='*60}")
            try:
                sample(args, subject_id=sid, group_id=gid)
            except Exception as e:
                print(f"[ERROR] subj {sid:02d} [{gtag}]: {e}")
