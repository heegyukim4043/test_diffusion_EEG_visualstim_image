...
class PatchEmbed1D(nn.Module):
    ...
    def __init__(self, time_len=224, patch_size=1, in_chans=128, embed_dim=256):
        ...
        print(f"[PatchEmbed1D] time_len={time_len}, patch_size={patch_size}, num_patches={self.num_patches}")
...
class MAEforEEG(nn.Module):
    ...
    def __init__(self, time_len=512, patch_size=4, embed_dim=1024, in_chans=128,
        ...
        if use_nature_img_loss:
            ...
            # 여기서 num_patches 대신 self.patch_embed.num_patches 사용
            self.nature_img_decoder_pred = nn.Sequential(
                nn.Conv1d(self.patch_embed.num_patches, 512, kernel_size=1, stride=1, bias=True),
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x)
        print(f"[forward_encoder] input={x.shape}, pos_embed={self.pos_embed[:, 1:, :].shape}")
...
class eeg_encoder(nn.Module):
    def __init__(self, time_len=512, patch_size=4, embed_dim=1024, in_chans=128,
        ...
        print("patch_embed.num_patches =", self.patch_embed.num_patches)
...
if __name__ == '__main__':
    ...
    # print(loss)
    encoder = eeg_encoder(time_len=512, patch_size=4, in_chans=128, embed_dim=1024)
    out = encoder(input)
    print(out.shape)
