...
from .datasets import MyEEGDataset
...
def create_EEG_dataset(args, split):
    if getattr(args, "dataset", None) == "MYEEG":
        return MyEEGDataset(root=args.root_path, split_key=args.split_key, split=split, image_size=512)
    # ??湲곗〈 濡쒖쭅(怨듭떇 ?곗씠?곗뀑)? 洹몃?濡??먭린
    ...
...
def create_EEG_dataset_legacy(eeg_signals_path='../dreamdiffusion/datasets/eeg_5_95_std.pth',
