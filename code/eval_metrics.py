...
# 기존: from torchmetrics.functional import accuracy   # <- 제거
from torchmetrics.functional.classification import accuracy as tm_accuracy
...
def n_way_top_k_acc(pred, class_id, n_way, num_trials=40, top_k=1):
    ...
    for t in range(num_trials):
        ...
        logits = pred_picked.unsqueeze(0)  # [1, n_way]
        target = torch.tensor([0], device=pred.device)

        acc = accuracy(
            preds=pred_picked.unsqueeze(0),  # shape [1, n_way]
            target=torch.tensor([0], device=pred_picked.device),
            task="multiclass",
            num_classes=n_way,
            top_k=top_k
        )
        acc_list.append(acc.item() if torch.is_tensor(acc) else float(acc))

        # acc_scalar = acc.item() if isinstance(acc, torch.Tensor) else float(acc)
        # acc_list.append(acc_scalar)
