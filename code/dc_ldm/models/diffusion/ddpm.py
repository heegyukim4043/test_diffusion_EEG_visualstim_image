...
from ...modules.encoders.modules import FrozenImageEmbedder
...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        x = x.to(memory_format=torch.channels_last).float()
        return x
    ...
    def training_step(self, batch, batch_idx):
        self.train()
        self.cond_stage_model.train()  ###?겼틫??쑉?ら뇤溫?퍌??        
    ...
    def generate(self, data, num_samples, ddim_steps=300, HW=None, limit=None, state=None):
        ...
        with model.ema_scope():
            for count, (eeg_sample, img_tensor) in enumerate(zip(data['eeg'], data['image'])):
                if limit is not None and count >= limit:
                    break

                # ------------------------------------------------------------------
                # 1) EEG는 일단 무시하고, 텍스트 프롬프트로만 조건 줌
                #    (지금 cond_stage_model 이 CLIP 텍스트 인코더이기 때문)
                # ------------------------------------------------------------------
                prompts = [f"an object for EEG sample {count}"] * num_samples

                # 안전하게 device 지정
                device = next(model.parameters()).device  # model의 디바이스 (보통 'cuda:0')
                c, re_latent = model.get_learned_conditioning(prompts)

                # cond 텐서가 CPU에 있을 수도 있으니 강제로 GPU로 이동
                if isinstance(c, dict):
                    for k in c.keys():
                        if isinstance(c[k], list):
                            c[k] = [x.to(device) for x in c[k]]
                        else:
                            c[k] = c[k].to(device)
                else:
                    c = c.to(device)
                ...
                # ------------------------------------------------------------------
                # 2) GT 이미지는 이미 (C, H, W) 라고 가정 → 배치 차원만 추가
                # ------------------------------------------------------------------
                # img_tensor: (3, H, W)
                gt_image = img_tensor.unsqueeze(0)  # (1, 3, H, W)
                ...
                x_samples_ddim = self._safe_decode(samples_ddim, chunk=1, dtype=None)

                # [-1,1] 범위 가정 → [0,1] 로 클램프
                x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
                gt_image      = torch.clamp((gt_image      + 1.0) / 2.0, min=0.0, max=1.0)

                # GT 를 맨 앞에 두고, 그 뒤에 샘플들
                all_samples.append(
                    torch.cat(
                        [gt_image.detach().cpu(), x_samples_ddim.detach().cpu()],
                        dim=0
                    )
                )

        # display as grid
        grid = torch.stack(all_samples, 0)          # (N, 1+num_samples, C, H, W)
        grid = rearrange(grid, 'n b c h w -> (n b) c h w')
        grid = make_grid(grid, nrow=num_samples + 1)

        # to image
        grid = 255. * rearrange(grid, 'c h w -> h w c').cpu().numpy()
        all_samples_np = (255. * torch.stack(all_samples, 0).cpu().numpy()).astype(np.uint8)

        return grid, all_samples_np, state
    ...
    def full_validation(self, batch, state=None):
        ...
        grid, all_samples, state = self.generate(batch, ddim_steps=20, num_samples=1, limit=3)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.validation_count % 5 == 0 and self.trainer.current_epoch != 0:
            ...
            grid, all_samples, state = self.generate(batch, ddim_steps=25, num_samples=1, limit=2)
    ...
    def get_eval_metric(self, samples, avg=True):
        ...
        res_list = []
        device_for_metric = 'cuda' if torch.cuda.is_available() else 'cpu'
        ...
        for s in samples_to_run:
            ...
            res = get_similarity_metric(pred_images, gt_images, 'class', None, 
                            n_way=10, num_trials=10, top_k=1, device=device_for_metric)
            res_part.append(np.mean(res))
        res_list.append(np.mean(res_part))
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # 항상 속성부터 만들어 둔다 (없는 속성 오류 방지)
        self.image_embedder = None

        # config/kwargs에서 clip_tune 읽기 (없으면 False)
        self.clip_tune = bool(kwargs.pop("clip_tune", getattr(self, "clip_tune", False)))

        # 켜져 있으면 즉시 생성 + 디바이스 이동
        if self.clip_tune and self.image_embedder is None:
            self.image_embedder = FrozenImageEmbedder().to(self.device)

        # 방어적 체크 (켜져 있는데 없으면 바로 에러 메시지로 중단)
        if self.clip_tune and self.image_embedder is None:
            raise RuntimeError("clip_tune=True 인데 image_embedder 초기화 실패")
        ...
        # ------------------------------------------------------
        # add: latent shape info for DDPM.generate()
        # ------------------------------------------------------
        # latent 채널 수 (보통 Stable Diffusion은 4)
        if not hasattr(self, "p_channels"):
            if hasattr(self.model.diffusion_model, "in_channels"):
                self.p_channels = self.model.diffusion_model.in_channels
            else:
                self.p_channels = self.channels  # fallback

        # latent spatial size (이미지 512, 다운샘플 8배 → 64)
        if not hasattr(self, "p_image_size"):
            # first_stage_config.params.ddconfig.ch_mult 길이로 down수 구해둔 게 이미 있음
            factor = 2 ** getattr(self, "num_downs", 3)  # 기본 3 → 2^3 = 8
            self.p_image_size = self.image_size // factor

        # ch_mult도 없는 경우를 대비해 설정
        if not hasattr(self, "ch_mult"):
            try:
    ...
    @torch.no_grad()
    def _safe_decode(self, z, chunk=1, dtype=None):
        # VAE 인코딩에서 scale_factor를 곱했으니, decode 전에 반드시 되돌리기
        z = (1.0 / self.scale_factor) * z

        xs = []
        fsm = self.first_stage_model
        was_training = fsm.training
        fsm.eval()
        try:
            for i in range(0, z.shape[0], chunk):
                ...
                if dtype is not None and torch.cuda.is_available():
                    ...
                    xi = fsm.decode(zi)
                xs.append(xi.float().cpu())
                del zi, xi
                torch.cuda.empty_cache()
        finally:
            fsm.train(was_training)
        return torch.cat(xs, dim=0)

    @rank_zero_only
    @torch.no_grad()
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx=None, *args, **kwargs):
    ...
    def get_learned_conditioning(self, c):
        ...
        if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
            out = self.cond_stage_model.encode(c)
        else:
            out = self.cond_stage_model(c)
    ...
    # ddpm.py (모델 클래스 안이면 self.first_stage_model 사용 가능)
    def decode_vae_chunked(self, z, chunk=1, dtype=torch.float16):
        z = (1.0 / self.scale_factor) * z
        xs = []
        fsm = self.first_stage_model
        was_training = fsm.training
        ...
        try:
            for i in range(0, z.shape[0], chunk):
                ...
                with torch.autocast(device_type="cuda", dtype=dtype):
                    xi = fsm.decode(zi)  # VAE decode
                torch.cuda.synchronize()
                xs.append(xi.float().cpu())  # 바로 CPU로
                del zi, xi
                torch.cuda.empty_cache()
        finally:
            fsm.train(was_training)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        img_raw = batch['image_raw'] if 'image_raw' in batch else x
        out = [z, c, batch['label'], img_raw]
    ...
    def forward(self, x, c, label, image_raw, *args, **kwargs):
        ...
        if self.clip_tune:
            if self.image_embedder is None:
                print("[warn] image_embedder missing; constructing now")
                self.image_embedder = FrozenImageEmbedder().to(self.device)
        ...
        """
        if self.clip_tune:
            img_for_clip = (image_raw + 1.0) / 2.0 if image_raw.min() < -0.1 else image_raw
            image_embeds = self.image_embedder(img_for_clip)
            loss_clip = self.cond_stage_model.get_clip_loss(re_latent, image_embeds)
            loss += loss_clip
            loss_dict.update({f'{prefix}/loss_clip': loss_clip})
    ...
    def configure_optimizers(self):
        ...
        opt = torch.optim.AdamW(uniq, lr=lr)
        return opt
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        self._ctx_dim_target = None
        self._cond_proj = None
        # 흔히 'context_dim'을 갖고 있으니 먼저 시도
        for attr in ["context_dim", "cond_dim", "transformer_context_dim"]:
            if hasattr(self.diffusion_model, attr):
                self._ctx_dim_target = getattr(self.diffusion_model, attr)
                break
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            # 보통 리스트 길이 1이므로 concat은 의미가 없지만, 호환 위해 유지
            cc = torch.cat(c_crossattn, dim=1) if isinstance(c_crossattn, list) else c_crossattn

            # ⬇ 추가: 차원 자동 보정(첫 호출 때 투사기 생성)
            ctx_in = cc.shape[-1]
            if self._ctx_dim_target is None:
                # UNet이 내부에서 기대하는 dim을 명시하지 않는 경우가 있어, 현재 dim을 기준으로 고정
                self._ctx_dim_target = ctx_in
            if ctx_in != self._ctx_dim_target:
                if self._cond_proj is None:
                    self._cond_proj = nn.Linear(ctx_in, self._ctx_dim_target, bias=False).to(cc.device)
                cc = self._cond_proj(cc)
...
class EEGClassifier(pl.LightningModule):
    ...
    def forward(self, x, c, label, image_raw, *args, **kwargs):
        ...
        if self.clip_tune and (self.global_step % 8 == 0):
            if self.image_embedder is None:
                self.image_embedder = FrozenImageEmbedder()
            image_embeds = self.image_embedder(image_raw)
            loss_clip = self.cond_stage_model.get_clip_loss(re_latent, image_embeds)
    ...
    def get_input(self, batch, k='image', return_first_stage_outputs=False, force_c_encode=False,
        ...
        img_raw = batch['image_raw'] if 'image_raw' in batch else x  # fallback
        out = [x, c, batch['label'], img_raw]
        # if return_first_stage_outputs:
        #     xrec = self.decode_first_stage(z)
        #     out.extend([x, xrec])
        # if return_original_cond:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 가벼운 샘플만
        grid, all_samples, state = self.generate(
            batch, ddim_steps=10, num_samples=1, limit=1
        )
    ...
    def get_learned_conditioning(self, c, num_samples=1):
        """
        c: latent tensor (B, L, D) or (L, D)
        num_samples: how many repetitions per sample
        """
        # 입력 정리
        if c.dim() == 2:
            # (L, D) -> (1, L, D)
        ...
        if num_samples > 1:
            ...
            c = repeat(c, 'b l d -> (b r) l d', r=num_samples)  # 배치 확장만

        # cond_stage_model forward
        if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
