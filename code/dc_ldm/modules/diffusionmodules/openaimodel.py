...
def linear(in_dim, out_dim, bias=True):
    """Wrapper around nn.Linear to match expected API."""
    return nn.Linear(in_dim, out_dim, bias=bias)

def conv_nd(dims, *args, **kwargs):
    """
    Create a 1D, 2D, or 3D convolution depending on dims.
    """
    if dims == 1:
...
# Zero-initialized module (common for residual connections)
...
# Timestep embedding (positional encoding for diffusion steps)
def timestep_embedding(timesteps, dim, max_period=10000):
    """
    Create sinusoidal timestep embeddings.
    - timesteps: Tensor [batch]
