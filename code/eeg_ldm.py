...
def main(config):
    ...
    img_transform_train = transforms.Compose([
        transforms.Resize((512, 512)),  # PIL 상태에서 Resize
    ...
    if config.dataset == 'EEG':
        pass
    elif config.dataset == 'MYEEG':
        from .datasets.my_eeg import MyEEGDataset
        eeg_latents_dataset_train = MyEEGDataset(
            root=config.root_path, split_key=config.split_key, split='train',
            transform=img_transform_train
        )
        eeg_latents_dataset_test = MyEEGDataset(
            root=config.root_path, split_key=config.split_key, split='test',
            transform=img_transform_test
        )
        num_voxels = eeg_latents_dataset_train[0]['eeg'].shape[0]
    else:
        raise NotImplementedError(f"Dataset {config.dataset} not supported")

    # 여기서 pretrain_mbm_metafile 은 None 이어도 가능
    generative_model = eLDM(
        ...
        use_time_cond=config.use_time_cond, clip_tune=config.clip_tune, cls_tune=config.cls_tune
    )
...
def get_args_parser():
    import argparse
    parser = argparse.ArgumentParser(description="Double Conditioning LDM Finetuning")

    # --- 기본 설정 ---
    parser.add_argument('--seed', type=int, default=2022)

    # --- 경로/데이터 ---
    parser.add_argument('--root_path', type=str, default='.', help='project root')
    parser.add_argument('--dataset', type=str, default='MYEEG', choices=['MYEEG'])
    parser.add_argument('--split_key', type=str, default='stim', choices=['all', 'stim', 'imag'])
...
def create_trainer(num_epoch, precision=16, accumulate_grad_batches=2,logger=None,check_val_every_n_epoch=0):
    ...
    return pl.Trainer(accelerator=acc, max_epochs=num_epoch, logger=logger, 
            ...
            check_val_every_n_epoch=check_val_every_n_epoch,num_sanity_val_steps=0)
  
if __name__ == '__main__':
    import torch.multiprocessing as mp

    try:
        mp.set_start_method("spawn", force=True)  # 윈도우 기본이 spawn이긴 하지만 명시
    except RuntimeError:
        pass
